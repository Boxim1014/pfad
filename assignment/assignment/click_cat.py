...
# 加载并放大小猫图像
cat_image_original = pygame.image.load(r"E:\POLYU_SEM1\SD5913-Gio\assignment2\cat.png")
cat_image_original = pygame.transform.scale(cat_image_original, (150, 150))

# 加载音效
click_sound = pygame.mixer.Sound(r"E:\POLYU_SEM1\SD5913-Gio\assignment2\Touch Sound _Cute.mp3")

# 小猫的属性
cat_pos = [random.randint(0, width - 150), random.randint(0, height - 150)]
click_count = 0
click_target = random.randint(5, 15)
shake_duration = 0.1  # 震动持续时间
last_shake_time = 0
...
while running:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            # 将当前时间、位置和随机偏移量存储为元组
            hearts.append((x, y, time.time(), random.uniform(-1.0, 1.0)))  # 调整幅度
    ...
    if time.time() - last_shake_time < shake_duration:
        scale_factor = 1.1  # 放大倍率
    else:
        scale_factor = 1.0  # 正常大小
